...
lat <- locs$latitude
long <- locs$longitude
tz <- attr(apaevlcr$DateTimeStamp, 'tzone')
```
...
obsplo <- reactive({
  ...
  toadd <- apaevlcr[rowsel, 'DateTimeStamp']
  out <- obsbs + 
...
corval <- reactive({
  ...
  out <- evalcorstp(apaevlcr, rowsel, tz, lat, long, corv = TRUE) %>% 
    round(2)
...
corplo <- reactive({
  ...
  vls <- evalcorstp(apaevlcr, rowsel, tz, lat, long, corv = FALSE)
...
## Comparing results with floored sun angle

This simple plot shows results from the standard `evalcor` function and a modified version that floors the sun angle estimate at zero (i.e., values below the horizone are treated as zero/no solar radiation to stimulate production).
